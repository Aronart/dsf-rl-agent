...
from prepare_data_fmp import get_company_profile
...
def sample_items(items: List[Dict], k: int = 5) -> List[Dict]:
    """
    Returns a random sample of k items from a list.
    If the list has k or fewer items, it returns the whole list.
    """
    if len(items) <= k:
        return items
...
def get_press_releases_for_date(row: Dict) -> List[Dict]:
    # Get the current row's date in 'YYYY-MM-DD' format
    date = row['Date'] if isinstance(row['Date'], str) else row['Date'].strftime('%Y-%m-%d')
    ...
    try:
        ...
        return [] # Return empty list if JSON is invalid or not a string

    # --- CORRECTED LOGIC IS HERE ---
    # The function now correctly handles the 'YYYY-MM-DD HH:MM:SS' date format from your FMP data

    # We prepare the target date for comparison by removing hyphens (e.g., '2020-07-16' -> '20200716')
    target_date_str = date.replace('-', '')

    filtered_releases = []
    for pr in press_releases:
        # For each press release, take the date part, remove hyphens, and then take the first 8 characters
        pr_date_str = pr.get('date', '').replace('-', '')[:8]

        # Now the comparison is between two 'YYYYMMDD' strings, which works correctly
        if pr_date_str and pr_date_str <= target_date_str:
...
def prepare_input(row: Dict, next_row: Dict, profile: Dict) -> Dict:
    ...
    sampled_news = sample_items(news, k=10)

    # --- START: HEURISTIC LOGIC FOR PRESS RELEASES ---
    
    # Calculate the average length of press release descriptions
    if not press_releases:
        avg_pr_length = 0
    else:
        total_pr_length = sum(len(pr.get('description', '')) for pr in press_releases)
        avg_pr_length = total_pr_length / len(press_releases)

    # Set the number of samples (k) based on the average length
    if avg_pr_length > 2000:  # If they are very long on average
        pr_k = 2
    elif avg_pr_length > 750: # If they are moderately long
        pr_k = 5
    else: # If they are short
        pr_k = 10

    sampled_press_releases = sample_items(press_releases, k=pr_k)
    
    # --- END: HEURISTIC LOGIC FOR PRESS RELEASES ---
    # sampled_press_releases = sample_items(press_releases, k=10)
    ...
    return {
        ...
        "press_releases": format_press_releases(sampled_press_releases),
        ...
        for i in tqdm(range(len(df) - 1), total=len(df) - 1):
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    print(f"MODEL'S RAW (MALFORMED) RESPONSE:\n{response}") 
                    print("="*54 + "\n")
